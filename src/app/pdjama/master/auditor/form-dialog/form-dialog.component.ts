...
export class FormDialogComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup = this.formBuilder.group({
      ...
      id_kelompok: ["", Validators.required],
      id: [""],
      kode_referral: [""],
  ...
  submitRequest(): void {
    ...
    const payload = {
      ...
      path_to_barcode: value.path_to_tanda_tangan,
    };

    let endpoint = "/pdjama/master/auditor/create";
  ...
  toRoute(data) {
    if (data) {
      ...
      this.dataService
        .getPostRequest<FormResponse>("/pengguna/kelompok/create", payload)
        .subscribe(
          (success) => {
            if (success.code == "404") {
              swal.fire({
            ...
            } else if (success.message.match(/Duplicate entry.*/)) {
              swal
                .fire({
                  title: "Tambah Data",
