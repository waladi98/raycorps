...
export class FormDialogComponent implements OnInit {
  ...
  loadDataTahunPeriode(): Promise<any> {
    return this.dataService
      .getRequest<any>("/pdjama/master/tahunPeriode", {where: "id_aktif = 'Y'"})
      .toPromise();
  }
  ...
  submitRequest(): void {
    ...
    const payload = {
      ...
      nama_auditor_1: value.nama_auditor_1,
      nama_auditor_2: value.nama_auditor_2,
      nama_auditor_3: value.nama_auditor_3,
      nama_audit: value.nama_audit,
      tanggal_audit: this.formatDate(value.tanggal_audit),
      dibuat_oleh: this.user,
    };

    let endpoint = "/pdjama/auditInternal/createx";
  ...
  getDataSekolah(search, nameLoader) {
    let where = "nama_bidang LIKE  '%" + search + "%' or singkatan LIKE  '%" + search + "%'";
