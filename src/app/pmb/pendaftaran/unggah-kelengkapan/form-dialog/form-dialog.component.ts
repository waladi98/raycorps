...
export class FormDialogComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup = this.formBuilder.group({
      ...
      kelengkapan: [{ value: "", disabled: true }],
  ...
  async inisialisasiDataFormCamaba(data) {
    ...
    this.formGroup.get("kelengkapan").setValue(data.kelengkapan);
    this.dokumen_kelengkapan = this.formGroup.get("kelengkapan").setValue(data.kelengkapan);
  ...
  submitRequest(): void {
    ...
    let endpoint = "/pmb/pesertaKelengkapan/modify";

    this.dataService.getPostRequest<FormResponse>(endpoint, this.payload).subscribe(
      (success) => {
  ...
  manageData(data) {
    if (data) {
      this.router.navigate(["/pmb/pendaftaran/unggah-kelengkapan"]);
    }
  }
  openFileUploader(): void {
    this.fileUploaderService
      .open({
        ...
        uploadParams: {
          path: 'kelengkapan/'+ this.dataPeserta.peserta_id,
        },
      ...
      .subscribe((result) => {
        if(result.data){
          ...
          this.formGroup.get('path_dokumen').setValue(result.data.result.newfilename);
        } 
      });
  }
