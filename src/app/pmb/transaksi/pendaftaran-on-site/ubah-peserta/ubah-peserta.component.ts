...
@Component({
  selector: "app-ubah-peserta",
  templateUrl: "ubah-peserta.component.html",
  styles: [
    `
      md-calendar {
        width: 300px;
...
export class UbahPesertaComponent implements OnInit {
  simpleSlider = 40;
  doubleSlider = [20, 60];
    ...
    swal
      ...
      .then((result) => {
        if (result.value) {
          ...
          swal.fire({
            title: 'Dibatalkan',
            text: 'Peserta Tidak Jadi Dihapuskan',
            icon: 'error',
            customClass: {
      ...
      .subscribe(
        (success) => {
          if (success.message == "Invalid Parameter") {
            swal.fire({
              title: "Ubah Peserta",
              text: "Data Peserta Gagal di Ubah.",
              icon: "error",
              customClass: {
