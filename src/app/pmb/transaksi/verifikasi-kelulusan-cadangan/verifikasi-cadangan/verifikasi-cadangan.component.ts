...
@Component({
  selector: "app-verifikasi-cadangan",
  templateUrl: "verifikasi-cadangan.component.html",
  styleUrls: ["./verifikasi-cadangan.component.scss"],
  styles: [
    `
      md-calendar {
        width: 300px;
...
export class VerifikasiCadanganComponent implements OnInit {
  listData = {
    header: [],
    field: [],
    action: [],
  ...
  async inisialisasiDataFormPeserta(data) {
    ...
    this.formGroup
      ...
      this.formGroup
      .get("catatan_kelulusan")
      .setValue(data.catatan_kelulusan);
        ...
        .subscribe(
          (success) => {
            if (success.message == "Invalid Parameter") {
              ...
              this.router.navigate(["/pmb/transaksi/proses-cadangan"]);
            }
          },
          (error) => {}
        );
