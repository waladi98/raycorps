...
export class FormDialogComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup = this.formBuilder.group({
      ...
      path_to_dokumen: [{ value: "", disabled: true }, Validators.required],
      keterangan_undur_diri: [{ value: "", disabled: true }],
      alasan_pengunduran_diri: [{ value: "", disabled: true }],
      id_status_dokumen: [{ value: "" }, Validators.required],
      status_mundur: [{ value: "" }],
    });
  ...
  async loadInitialData(): Promise<any> {
    try {
      ...
      const [dataPeserta, listDataStatusDokumen, dataTRNPeserta] =
      ...
      this.dataTRNPeserta =
        dataTRNPeserta.result.length > 0 ? dataTRNPeserta.result[0] : false;
      // console.log("WAL", this.dataTRNPeserta);
      this.hideSpinner();
    } catch (error) {
      this.hideSpinner();
      console.log(error);
  ...
  loadTRNPeserta(): Promise<any> {
    return this.dataService
      .getRequest<any>("/pmb/peserta", {
        where: "formulir_id='" + this.dataParam.type.formulir_id + "'",
      })
      .toPromise();
  }
  ...
  async inisialisasiDataFormCamaba(data) {
    ...
    this.formGroup.get("status_mundur").setValue("Y");
  }
  ...
  toRoute(data) {
    if (data) {
      const payload = {
        id: this.dataTRNPeserta.id,
        status_mundur: this.formGroup.get("status_mundur").value,
      };
              ...
              swal
                ...
                .then((result) => {
                  this.router.navigate([
                    "/pmb/transaksi/proses-pengunduran-diri",
                  ]);
  ...
  cancelUpload() {
    this.dataPeserta.path_to_dokumen = null;
